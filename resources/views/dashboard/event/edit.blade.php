...
@section('container')
    ...
    <form method="post" action="/dashboard/event/{{$event->id}}" class="mb-5" enctype="multipart/form-data">
        ...
        @csrf
            <div class="mb-3">
                <label for="title" class="form-label">Aktivity</label>
                <input type="text" class="form-control @error('title') is-invalid @enderror" id="title" name="title"  autofocus value="{{old('content', $event->title)}}">
                @error('title')
            ...
            <div class="mb-3">
                ...
                @if($event->image)
                <img src="{{ asset('storage/' . $event->image) }}" class="img-preview img-fluid mb-3 col-sm-5 d-flex">
                @else
